...
struct AdService {
    ...
    static func fetch(for uid: String, in game: String, completion: @escaping (Ad?) -> Void) {
        let ref = Database.database().reference().child(Constants.Database.ads).child(game).child(uid)
        ref.observeSingleEvent(of: .value, with: { snapshot in
            return completion(Ad(snapshot))
        })
    }
